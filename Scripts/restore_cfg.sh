...
print_log -g "[python env]" -b " :: " "Rebuilding 404 Python environment..."
if command -v 404-shell >/dev/null 2>&1; then
    404-shell pyinit
else
    "${HOME}/.local/bin/404-shell" pyinit
fi
...
clone_dir=$(git rev-parse --show-toplevel 2>/dev/null || echo "${HOME}/404")
