...
if [ -d "${HOME}/.config/nvim" ] && [ -f "${HOME}/.config/nvim/lua/lazyvim" ]; then
...
if [ -d "${HOME}/.config/nvim" ] && [ ! -d "${HOME}/.config/nvim/lua/lazyvim" ]; then
...
if [ ${flg_DryRun:-0} -eq 0 ]; then
    ...
    if [ $? -eq 0 ]; then
        ...
        print_log -r "[LAZYVIM] " -b "failed :: " "Could not install LazyVim"
        exit 1
    fi
else
