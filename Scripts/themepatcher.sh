...
wallbashDirs=(
    ...
    "/usr/local/share/404/wallbash"
    "/usr/share/404/wallbash"
)
...
if [ -d "$2" ]; then
    ...
    Git_Owner=${Git_Path%/*}
    branch_dir=${branch//\//_}
    cacheDir=${cacheDir:-"$HOME/.cache/404"}
...
Fav_Theme_Dir="${Theme_Dir}/Configs/.config/404/themes/${Fav_Theme}"
[ ! -d "${Fav_Theme_Dir}" ] && print_prompt -r "[ERROR] " "'${Fav_Theme_Dir}'" -y " Do not Exist" && exit 1

# config=$(find "${dcolDir}" -type f -name "*.dcol" | awk -v favTheme="${Fav_Theme}" -F 'theme/' '{gsub(/\.dcol$/, ".theme"); print ".config/404/themes/" favTheme "/" $2}')
config=$(find "${wallbashDirs[@]}" -type f -path "*/theme*" -name "*.dcol" 2>/dev/null | awk '!seen[substr($0, match($0, /[^/]+$/))]++' | awk -v favTheme="${Fav_Theme}" -F 'theme/' '{gsub(/\.dcol$/, ".theme"); print ".config/404/themes/" favTheme "/" $2}')
restore_list=""
...
if [ -f "${Fav_Theme_Dir}/theme.dcol" ]; then
    ...
    restore_list+="Y|Y|\${HOME}/.config/404/themes/${Fav_Theme}|theme.dcol|hyprland\n"
fi
...
# populate wallpaper
Fav_Theme_Walls="${confDir}/404/themes/${Fav_Theme}/wallpapers"
...
# populate logos
Fav_Theme_Logos="${confDir}/404/themes/${Fav_Theme}/logo"
if [ -n "${logos}" ]; then
...
if [ "${3}" != "--skipcaching" ]; then
    "$HOME/.local/lib/404/swwwallcache.sh" -t "${Fav_Theme}"
    "$HOME/.local/lib/404/theme.switch.sh"
fi
