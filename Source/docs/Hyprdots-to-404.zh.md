...
> **注意**: 如果你奇怪为什么每次 ```install.sh -r``` 都会覆盖您的配置，您应该 fork [404](https://github.com/404-Project/404)，编辑 ```*.lst``` 文件。这才是我们预期的方式。
...
接下来我们要谈谈如何在不改变用户配置的情况下更新 404 相关的配置文件。
我们不再需要 “userprefs” 文件。相对的，我们可以引用(source) 404 的 hyprland.conf 并允许\$USER 直接在配置中更改。通过这个方式，您不会破坏 404，404 也不会破坏您的配置文件。
